...
@WebServlet("/updateQotdController")
public class UpdateQOTDController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public UpdateQOTDController() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		qotd.setUpdateUser(updateUserIdAccount);
		qotd.setQuestion(request.getParameter("question"));
		qotd.setIdQuestion(Integer.parseInt(request.getParameter("idQuestion")));
		
		try {
			
			new QOTDOperations();
			
			if (QOTDOperations.updateQOTD(qotd)) {
				//response.sendRedirect("manila/adminQOTD.jsp");
				
				new TransactionLogOperations();
				TransactionLogOperations.addTransactionLog(updateUserIdAccount, "updateQOTD", "updated a question of the day.",
